...
/**
 * Extractor for populating purely textual information about a file, namely its lines and their line terminators.
 */
public class TextualExtractor {
	private final static Pattern LINE_TERMINATOR = Pattern.compile("\\n|\\r\\n|\\r|\\u2028|\\u2029");
	private final static Pattern INDENT_CHAR = Pattern.compile("((\\s)\\2*)\\S.*");
	...
	public TextualExtractor(TrapWriter trapwriter, LocationManager locationManager, String source, boolean extractLines) {
	...
	public static String sanitiseToString(String str) {
		...
		for (int i=0, n=str.length(); i<n; ++i) {
			...
			if (c < 0x20 || c > 0x7e) {
				int j = esc.indexOf(c);
				if (j >= 0)
					res.append("\\" + esc.charAt(j+1));
				else
					res.append("\\u" + String.format("%04x", Integer.valueOf(c)));
			} else {
				res.append((char)c);
			}
		}
	...
	/**
	 * Emit a lines/4 tuple, if {@link #extractLines} is {@code true},  and an
	 * indentation/4 tuple, where applicable, for a line of source code.
	 *
	...
	public String getLine(int lineNumber) {
		...
		while (matcher.find()) {
			if (n == lineNumber)
				return source.substring(end, matcher.end());
			++n;
			end = matcher.end();
		}
