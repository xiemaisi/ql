...
public class JSONExtractor implements IExtractor {
	...
	public LoCInfo extract(final TextualExtractor textualExtractor) {
		...
		try {
			...
			v.accept(new Visitor<Context, Label>() {
				private Label emit(JSONValue nd, int kind, Context c) {
					Label label = trapwriter.localID(nd);
					trapwriter.addTuple("json", label, kind, c.parent, c.childIndex, textualExtractor.mkToString(nd));
				...
				public Label visit(JSONLiteral nd, Context c) {
					int kind = 0;
					if (nd.getValue() instanceof Boolean)
						kind = 1;
	...
	private void populateError(final TextualExtractor textualExtractor,
			final TrapWriter trapwriter, final LocationManager locationManager,
			ParseError e) {
