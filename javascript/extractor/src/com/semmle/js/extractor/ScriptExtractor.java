...
public class ScriptExtractor implements IExtractor {
	...
	public LoCInfo extract(TextualExtractor textualExtractor) {
		...
		if (source.startsWith("#!")) {
			...
			if (config.getPlatform() != Platform.WEB)
				config = config.withPlatform(Platform.NODE);
		...
		LoCInfo loc;
		try {
			Pair<Label, LoCInfo> res = new JSExtractor(config).extract(textualExtractor, source, 0, scopeManager);
